...
struct Maxtrix {
    ...
    func isLegalIndex(row: Int, col: Int) -> Bool {
        return row >= 0 && row < rows && col >= 0 && col < columns
    }
    
    subscript(row: Int, col: Int) -> Int {
        get {
            assert(isLegalIndex(row: row, col: col), "数组索引越界")
            return grid[col + (row * columns)]
        }
        
        set {
            assert(isLegalIndex(row: row, col: col), "数组索引越界")
            grid[col + (row * columns)] = newValue
        }
    }
}

var matrix1 = Maxtrix(rows: 2, cols: 3)
matrix1[0,0] = 1
matrix1[1,2] = 2
