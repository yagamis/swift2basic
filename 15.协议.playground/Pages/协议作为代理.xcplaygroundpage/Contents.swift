...
struct GameAgent: Lolgamedelegate {
    func gameStart(game: Lolgame) -> Int {
        print(game.role.name,"开始打怪!","起始经验:2000")
        return 2000
    }
    ...
    func gameRolePK(game: Lolgame, armed: Bool) -> Int {
        if armed {
            ...
            print(game.role.name,"您赤手空拳,只获的经验:2500,有武器加倍哦")
            return 2500
        }
    }
    
    func gameOver(game: Lolgame) -> Int {
        print(game.role.name,"您已正常退出,获得经验1000")
        return 1000
    }
}
...
var playgame1 = PlayGame(exp: 0, gameAgent: agentSmith)
playgame1.play()
